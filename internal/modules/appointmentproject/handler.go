...
func (h *Handler) Create(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			response.BadRequest(c, "该服务项目已存在")
			return
		}
...
func (h *Handler) Update(c *gin.Context) {
	...
	var cid int64
	if err := h.db.QueryRow(c.Request.Context(), `SELECT calendar_id FROM appointment_projects WHERE id=$1 AND deleted_at IS NULL`, id).Scan(&cid); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			response.NotFound(c, "project not found")
			return
		}
		response.Internal(c, "query project failed")
		return
	}
	if !h.ensure(c, cid, uid) {
		response.Forbidden(c, "no permission")
	...
	if err != nil {
		...
		response.Internal(c, "update project failed")
		return
	}
	response.OK(c, gin.H{"id": id, "calendar_id": cid, "name": name})
}
func (h *Handler) Delete(c *gin.Context) {
	...
	_, err := h.db.Exec(c.Request.Context(), `UPDATE appointment_projects SET deleted_at=NOW(),updated_at=NOW() WHERE id=$1 AND deleted_at IS NULL`, id)
	if err != nil {
