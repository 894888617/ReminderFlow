...
func NewRouter(db *pgxpool.Pool, cfg *config.Config) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		authGroup.Use(middleware.JWTAuthMiddleware(cfg.JWTSecret))
		{
			...
			authGroup.POST("/calendars/:calendar_id/invites", inviteHandler.Create)
			...
			authGroup.POST("/calendars/:calendar_id/records", recordHandler.Create)
			authGroup.GET("/calendars/:calendar_id/records", recordHandler.List)
