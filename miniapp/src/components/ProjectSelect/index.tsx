...
export default function ProjectSelect({ calendarId, value, projectId, onChange }:{calendarId:number;value:string;projectId?:number|null;onChange:(v:{projectName:string;projectId:number|null})=>void}) {
  ...
  const filtered=useMemo(()=>items.filter(i=>i.name.includes(keyword.trim())),[items,keyword])
  ...
  const maybeCreate=async()=>{ const name=(value||'').trim(); if(!name||!calendarId) return; const exist=items.find(i=>i.name===name); if(exist){ onChange({projectId:exist.id,projectName:exist.name}); return } try{ const created=await createAppointmentProject(calendarId,name); onChange({projectId:created.id,projectName:created.name}) }catch(e){ console.log(e) } }
