...
export default function RecordEditPage() {
  ...
  const [customerId, setCustomerId] = useState<number | undefined>(); const [customerName, setCustomerName] = useState(''); const [customerPhone, setCustomerPhone] = useState(''); const [projectName, setProjectName] = useState(''); const [projectId, setProjectId] = useState<number | null>(null); const [saveToCustomer, setSaveToCustomer] = useState(false)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setCustomerId(detail.customer_id || undefined); setCustomerName(detail.customer_name || ''); setCustomerPhone(detail.customer_phone || ''); setProjectName(detail.project_name || detail.service_name || ''); setProjectId(detail.project_id || null)
  ...
  return <View className='container'><View className='form-card'>
    <View className='title-row'><Text className='form-label no-margin'>标题</Text>
    ...
    <View className='form-item'><Input className='form-input' value={title} onInput={(e) => setTitle(e.detail.value)} maxlength={200} placeholder='可留空自动生成 客户姓名 + 服务项目' /></View>
    <View className='grid-two'><View className='form-item compact-item'><Text className='form-label'>姓名</Text><Input className='form-input' value={customerName} onInput={(e) => setCustomerName(e.detail.value)} /></View><View className='form-item compact-item'><Text className='form-label'>手机号</Text><Input className='form-input' value={customerPhone} onInput={(e) => setCustomerPhone(e.detail.value)} /></View></View>
    <View className='grid-two'><View className='form-item compact-item'><Text className='form-label'>项目</Text><ProjectSelect calendarId={record?.calendar_id || 0} value={projectName} projectId={projectId} onChange={({projectName,projectId})=>{setProjectName(projectName);setProjectId(projectId)}} /></View><View className='form-item compact-item'><Text className='form-label'>负责人</Text><View className='assignee-box-wrap'>{memberOptions.length === 0 ? <View className='assignee-select-box placeholder'>暂无成员可选</View> : <Picker mode='selector' range={memberOptions.map((item) => item.label)} value={selectedAssigneeIndex >= 0 ? selectedAssigneeIndex : 0} onChange={(e) => { const selected = memberOptions[Number(e.detail.value)]; if (selected) setAssigneeId(selected.value) }}><View className={assigneeId ? 'assignee-select-box' : 'assignee-select-box placeholder'}>{selectedAssigneeName}</View></Picker>}{assigneeId ? <View className='assignee-clear' onClick={() => setAssigneeId(undefined)}>×</View> : null}</View></View></View>
